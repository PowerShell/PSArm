...
namespace PSArm
{
    ...
    {
        public IArmExpression ParseExpression(string s)
        {
            if (s.Length == 0)
            {
        ...
        private IArmExpression ParseWithTokenizer(string s)
        {
        ...
        private IArmExpression ParseFullExpression(ref ArmExpressionTokenizer tokenizer)
        {
        ...
        private IArmExpression ParseInnerExpression(ref ArmExpressionTokenizer tokenizer, bool noLiteral = false)
        {
            ...
            switch (token.Type)
            {
                case ArmExpressionTokenType.String:
                    ...
                    return new ArmStringLiteral(((ArmExpressionStringToken)token).Value);
        ...
        {
            var arguments = new List<IArmExpression>();
            ...
            while (!sawCloseParen)
            {
                ...
                switch (token.Type)
                {
                    ...
                    case ArmExpressionTokenType.String:
                        arguments.Add(new ArmStringLiteral(((ArmExpressionStringToken)token).Value));
                        break;

                    case ArmExpressionTokenType.Integer:
                        arguments.Add(new ArmIntLiteral(((ArmExpressionIntegerToken)token).Value));
                        break;

                    case ArmExpressionTokenType.Boolean:
                        arguments.Add(new ArmBoolLiteral(((ArmExpressionBooleanToken)token).Value));
                        break;
            ...
            return new ArmFunctionCall(identifier.Identifier, arguments.ToArray());
        }
        ...
        private ArmOperation ParseDotExpression(ref ArmExpressionTokenizer tokenizer, ArmOperation lhs)
        {
            ...
            switch (token.Type)
            {
                case ArmExpressionTokenType.Dot:
                    ...
                    {
                        case ArmExpressionTokenType.Identifier:
                            return ParseDotExpression(ref tokenizer, new ArmMemberAccess(lhs, ((ArmExpressionIdentifierToken)token).Identifier));
